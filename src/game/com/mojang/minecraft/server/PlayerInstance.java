...
/* THE FOLLOWING IMPORTS HAVE BEEN ADDED FOR SCUFFEDSERVER */
...
public final class PlayerInstance {
				...
				if(var1 != Packet.TIMED_OUT) {
					if(this.onlyIP && this.sendingPackets) {
						if(var1 == Packet.PLACE_OR_REMOVE_TILE) {
							/* THE FOLLOWING 9 LINES HAVE BEEN ADDED FOR SCUFFEDSERVER */
							if (!this.scuffedPlayer.loggedIn) {
								...
								this.sendChatMessage(Messages.NOT_LOGGED_IN);
								return;
							} else {
							...
							/* THE ABOVE LINE HAS BEEN ADDED FOR SCUFFEDSERVER */
	...
	private void chatMessage(String var1) {
		...
		/* THE FOLLOWING LINE HAS BEEN MODIFIED FOR SCUFFEDSERVER */
		if(this.chatCounter > 600 && !ScuffedUtils.isAdmin(this.minecraft, this.name)) {
			...
			if(var1.startsWith("/")) {
				...
				ScuffedMinecraftServer.getInstance().sendChat(this, var1);
			}
		}
	}
	...
	private void kickCheat(String var1) {
		/* THE FOLLOWING LINE HAS BEEN MODIFIED FOR SCUFFEDSERVER */
		if (ScuffedMinecraftServer.getInstance().antiCheat) this.kick("Cheat detected: " + var1);
	}
	...
	public final void handlePackets() {
		...
		if(!this.onlyIP && System.currentTimeMillis() - this.currentTime > 5000L) {
			...
			/* THE FOLLOWING 10 LINES HAVE BEEN MODIFIED FOR SCUFFEDSERVER */
	...
	public final void handleException(Exception var1) {
		...
		/* THE FOLLOWING LINE HAS BEEN MODIFIED FOR SCUFFEDSERVER */
		ScuffedMinecraftServer.getInstance().sendLogout(this.scuffedPlayer);
